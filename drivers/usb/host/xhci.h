 ...
 */
#if (defined(CONFIG_NLM_XLP) && defined(CONFIG_CPU_BIG_ENDIAN))
				 ...
				 const u64 val, __le64 __iomem *regs)
{
        ...
        xhci_dbg(xhci,
                        "`MEM_WRITE_DWORD(3'b000, 64'h%p, 64'h%0lx, 4'hf %p);\n",
                        regs, (long unsigned int) val, ptr);
#if (defined(CONFIG_NLM_XLP) && defined(CONFIG_CPU_BIG_ENDIAN))
        writel_be(val_lo, ptr);
        writel_be(val_hi, ptr + 1);
#else
        writel(val_lo, ptr);
        writel(val_hi, ptr + 1);
#endif
